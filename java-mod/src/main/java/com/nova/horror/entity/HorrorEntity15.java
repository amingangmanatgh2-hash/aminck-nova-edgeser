...
/**
 * HorrorEntity15 - Real horror entity 15
...
public class HorrorEntity15 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 95.0).add(Attributes.MOVEMENT_SPEED, 0.40).add(Attributes.ATTACK_DAMAGE, 11.0).add(Attributes.FOLLOW_RANGE, 40.0); }
    ...
    public void playNightSounds0_15() {
    ...
    public void teleportBehind3_15(Player player) {
    ...
    public void teleportBehind6_15(Player player) {
        if (player == null || teleportCooldown > 0) return;
        Vec3 look = player.getLookAngle();
        Vec3 toMob = new Vec3(getX() - player.getX(), 0, getZ() - player.getZ()).normalize();
        double dot = look.dot(toMob);
    ...
    public void playNightSounds7_15() {
        if (level.isClientSide) return;
        if (level.isNight() && tickCount % 80 == 0) {
            BlockPos pos = blockPosition();
            SoundEvent[] nights = {
    ...
    public void chasePlayer8_15(Player player) {
    ...
    public void playNightSounds11_15() {
        if (level.isClientSide) return;
        if (level.isNight() && tickCount % 80 == 0) {
            BlockPos pos = blockPosition();
            SoundEvent[] nights = {
    ...
    public void ambushInDark12_15(Player player) {
    ...
    public void ambushInDark14_15(Player player) {
        if (player == null) return;
        int light = level.getBrightness(LightLayer.BLOCK, player.blockPosition());
        if (light < 4 && distanceTo(player) < 6) {
            setInvisible(true);
