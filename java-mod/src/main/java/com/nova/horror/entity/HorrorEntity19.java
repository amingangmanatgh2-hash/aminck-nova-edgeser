...
/**
 * HorrorEntity19 - Real horror entity 19
...
public class HorrorEntity19 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 115.0).add(Attributes.MOVEMENT_SPEED, 0.44).add(Attributes.ATTACK_DAMAGE, 7.0).add(Attributes.FOLLOW_RANGE, 44.0); }
    ...
    public void dropJumpscare0_19(Player player) {
    ...
    public void dropJumpscare2_19(Player player) {
        if (player == null || !isOnCeiling) return;
        if (distanceTo(player) < 5 && random.nextInt(30)==0) {
            isOnCeiling = false;
            setNoGravity(false);
    ...
    public void ambushInDark5_19(Player player) {
    ...
    public void teleportBehind6_19(Player player) {
    ...
    public void ambushInDark7_19(Player player) {
        if (player == null) return;
        int light = level.getBrightness(LightLayer.BLOCK, player.blockPosition());
        if (light < 4 && distanceTo(player) < 6) {
            setInvisible(true);
    ...
    public void ambushInDark8_19(Player player) {
        if (player == null) return;
        int light = level.getBrightness(LightLayer.BLOCK, player.blockPosition());
        if (light < 4 && distanceTo(player) < 6) {
            setInvisible(true);
    ...
    public void chasePlayer13_19(Player player) {
        if (player == null || level.isClientSide) return;
