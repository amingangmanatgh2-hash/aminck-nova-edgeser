...
/**
 * HorrorEntity09 - Real horror entity 9
...
public class HorrorEntity09 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 65.0).add(Attributes.MOVEMENT_SPEED, 0.34).add(Attributes.ATTACK_DAMAGE, 5.0).add(Attributes.FOLLOW_RANGE, 34.0); }
    ...
    public void chasePlayer0_9(Player player) {
    ...
    public void dropJumpscare3_9(Player player) {
    ...
    public void dropJumpscare4_9(Player player) {
        if (player == null || !isOnCeiling) return;
        if (distanceTo(player) < 5 && random.nextInt(30)==0) {
            isOnCeiling = false;
            setNoGravity(false);
    ...
    public void teleportBehind6_9(Player player) {
    ...
    public void chasePlayer7_9(Player player) {
        if (player == null || level.isClientSide) return;
        double predX = player.getX() + player.getDeltaMovement().x * 15;
        double predZ = player.getZ() + player.getDeltaMovement().z * 15;
        Vec3 targetPos = new Vec3(predX, player.getY(), predZ);
    ...
    public void ambushInDark12_9(Player player) {
