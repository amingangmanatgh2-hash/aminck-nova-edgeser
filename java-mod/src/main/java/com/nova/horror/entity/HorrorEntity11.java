...
/**
 * HorrorEntity11 - Real horror entity 11
...
public class HorrorEntity11 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 75.0).add(Attributes.MOVEMENT_SPEED, 0.36).add(Attributes.ATTACK_DAMAGE, 7.0).add(Attributes.FOLLOW_RANGE, 36.0); }
    ...
    public void teleportBehind0_11(Player player) {
    ...
    public void chasePlayer1_11(Player player) {
    ...
    public void chasePlayer2_11(Player player) {
        if (player == null || level.isClientSide) return;
        double predX = player.getX() + player.getDeltaMovement().x * 15;
        double predZ = player.getZ() + player.getDeltaMovement().z * 15;
        Vec3 targetPos = new Vec3(predX, player.getY(), predZ);
    ...
    public void playNightSounds3_11() {
    ...
    public void teleportBehind5_11(Player player) {
        if (player == null || teleportCooldown > 0) return;
        Vec3 look = player.getLookAngle();
        Vec3 toMob = new Vec3(getX() - player.getX(), 0, getZ() - player.getZ()).normalize();
        double dot = look.dot(toMob);
    ...
    public void playNightSounds8_11() {
        if (level.isClientSide) return;
        if (level.isNight() && tickCount % 80 == 0) {
            BlockPos pos = blockPosition();
            SoundEvent[] nights = {
    ...
    public void ambushInDark11_11(Player player) {
    ...
    public void playNightSounds12_11() {
        if (level.isClientSide) return;
        if (level.isNight() && tickCount % 80 == 0) {
            BlockPos pos = blockPosition();
            SoundEvent[] nights = {
    ...
    public void teleportBehind13_11(Player player) {
        if (player == null || teleportCooldown > 0) return;
        Vec3 look = player.getLookAngle();
        Vec3 toMob = new Vec3(getX() - player.getX(), 0, getZ() - player.getZ()).normalize();
        double dot = look.dot(toMob);
    ...
    public void ambushInDark14_11(Player player) {
        if (player == null) return;
        int light = level.getBrightness(LightLayer.BLOCK, player.blockPosition());
        if (light < 4 && distanceTo(player) < 6) {
            setInvisible(true);
