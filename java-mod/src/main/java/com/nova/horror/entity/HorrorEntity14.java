...
public class HorrorEntity14 extends Monster {
    ...
    public void tick() {
        ...
        phase++;
        boolean isNight = level().isNight(); if (!isNight) { setDeltaMovement(0, getDeltaMovement().y, 0); getNavigation().stop(); if (phase % 200 == 0) level().playSound(null, blockPosition(), SoundEvents.STONE_BREAK, SoundSource.BLOCKS, 0.3F, 0.8F); } else { if (phase % 60 == 0) addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 80, 1)); if (getTarget() instanceof Player p && distanceTo(p) < 6 && cooldown==0) { level().playSound(null, blockPosition(), SoundEvents.WARDEN_ROAR, SoundSource.HOSTILE, 1.0F, 0.4F); p.addEffect(new MobEffectInstance(MobEffects.DARKNESS, 100, 0)); cooldown=200; } }
    }
