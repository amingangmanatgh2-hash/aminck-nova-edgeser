...
/**
 * HorrorEntity10 - Real horror entity 10
...
public class HorrorEntity10 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 70.0).add(Attributes.MOVEMENT_SPEED, 0.35).add(Attributes.ATTACK_DAMAGE, 6.0).add(Attributes.FOLLOW_RANGE, 35.0); }
    ...
    public void ambushInDark2_10(Player player) {
    ...
    public void dropJumpscare3_10(Player player) {
    ...
    public void dropJumpscare4_10(Player player) {
        if (player == null || !isOnCeiling) return;
        if (distanceTo(player) < 5 && random.nextInt(30)==0) {
            isOnCeiling = false;
            setNoGravity(false);
    ...
    public void teleportBehind5_10(Player player) {
    ...
    public void dropJumpscare7_10(Player player) {
        if (player == null || !isOnCeiling) return;
        if (distanceTo(player) < 5 && random.nextInt(30)==0) {
            isOnCeiling = false;
            setNoGravity(false);
    ...
    public void chasePlayer9_10(Player player) {
    ...
    public void playNightSounds10_10() {
    ...
    public void dropJumpscare12_10(Player player) {
        if (player == null || !isOnCeiling) return;
        if (distanceTo(player) < 5 && random.nextInt(30)==0) {
            isOnCeiling = false;
            setNoGravity(false);
    ...
    public void teleportBehind13_10(Player player) {
        if (player == null || teleportCooldown > 0) return;
        Vec3 look = player.getLookAngle();
        Vec3 toMob = new Vec3(getX() - player.getX(), 0, getZ() - player.getZ()).normalize();
        double dot = look.dot(toMob);
    ...
    public void ambushInDark14_10(Player player) {
        if (player == null) return;
        int light = level.getBrightness(LightLayer.BLOCK, player.blockPosition());
        if (light < 4 && distanceTo(player) < 6) {
            setInvisible(true);
