...
/**
 * HorrorEntity13 - Real horror entity 13
...
public class HorrorEntity13 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 85.0).add(Attributes.MOVEMENT_SPEED, 0.38).add(Attributes.ATTACK_DAMAGE, 9.0).add(Attributes.FOLLOW_RANGE, 38.0); }
    ...
    public void ambushInDark0_13(Player player) {
    ...
    public void dropJumpscare3_13(Player player) {
    ...
    public void teleportBehind5_13(Player player) {
    ...
    public void ambushInDark10_13(Player player) {
        if (player == null) return;
        int light = level.getBrightness(LightLayer.BLOCK, player.blockPosition());
        if (light < 4 && distanceTo(player) < 6) {
            setInvisible(true);
    ...
    public void chasePlayer12_13(Player player) {
        if (player == null || level.isClientSide) return;
    ...
    public void playNightSounds13_13() {
    ...
    public void chasePlayer14_13(Player player) {
        if (player == null || level.isClientSide) return;
        double predX = player.getX() + player.getDeltaMovement().x * 15;
        double predZ = player.getZ() + player.getDeltaMovement().z * 15;
        Vec3 targetPos = new Vec3(predX, player.getY(), predZ);
