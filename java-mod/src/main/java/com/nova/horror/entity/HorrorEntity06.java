...
/**
 * HorrorEntity06 - Real horror entity 6
...
public class HorrorEntity06 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 50.0).add(Attributes.MOVEMENT_SPEED, 0.31).add(Attributes.ATTACK_DAMAGE, 10.0).add(Attributes.FOLLOW_RANGE, 31.0); }
    ...
    public void chasePlayer0_6(Player player) {
    ...
    public void teleportBehind3_6(Player player) {
    ...
    public void playNightSounds5_6() {
    ...
    public void ambushInDark9_6(Player player) {
    ...
    public void teleportBehind10_6(Player player) {
        if (player == null || teleportCooldown > 0) return;
        Vec3 look = player.getLookAngle();
        Vec3 toMob = new Vec3(getX() - player.getX(), 0, getZ() - player.getZ()).normalize();
        double dot = look.dot(toMob);
    ...
    public void ambushInDark11_6(Player player) {
        if (player == null) return;
        int light = level.getBrightness(LightLayer.BLOCK, player.blockPosition());
        if (light < 4 && distanceTo(player) < 6) {
            setInvisible(true);
    ...
    public void playNightSounds12_6() {
        if (level.isClientSide) return;
        if (level.isNight() && tickCount % 80 == 0) {
            BlockPos pos = blockPosition();
            SoundEvent[] nights = {
    ...
    public void chasePlayer13_6(Player player) {
        if (player == null || level.isClientSide) return;
        double predX = player.getX() + player.getDeltaMovement().x * 15;
        double predZ = player.getZ() + player.getDeltaMovement().z * 15;
        Vec3 targetPos = new Vec3(predX, player.getY(), predZ);
    ...
    public void dropJumpscare14_6(Player player) {
