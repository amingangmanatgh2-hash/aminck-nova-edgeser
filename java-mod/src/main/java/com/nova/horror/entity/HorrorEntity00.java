...
/**
 * HorrorEntity00 - Real horror entity 0
...
public class HorrorEntity00 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 20.0).add(Attributes.MOVEMENT_SPEED, 0.25).add(Attributes.ATTACK_DAMAGE, 4.0).add(Attributes.FOLLOW_RANGE, 25.0); }
    ...
    public void playNightSounds3_0() {
    ...
    public void playNightSounds4_0() {
        if (level.isClientSide) return;
        if (level.isNight() && tickCount % 80 == 0) {
            BlockPos pos = blockPosition();
            SoundEvent[] nights = {
    ...
    public void chasePlayer6_0(Player player) {
        if (player == null || level.isClientSide) return;
    ...
    public void teleportBehind10_0(Player player) {
    ...
    public void dropJumpscare12_0(Player player) {
