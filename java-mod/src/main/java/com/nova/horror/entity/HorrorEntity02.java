...
/**
 * HorrorEntity02 - Real horror entity 2
...
public class HorrorEntity02 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 30.0).add(Attributes.MOVEMENT_SPEED, 0.27).add(Attributes.ATTACK_DAMAGE, 6.0).add(Attributes.FOLLOW_RANGE, 27.0); }
    ...
    public void ambushInDark1_2(Player player) {
    ...
    public void ambushInDark3_2(Player player) {
        if (player == null) return;
        int light = level.getBrightness(LightLayer.BLOCK, player.blockPosition());
        if (light < 4 && distanceTo(player) < 6) {
            setInvisible(true);
    ...
    public void chasePlayer5_2(Player player) {
    ...
    public void teleportBehind7_2(Player player) {
    ...
    public void dropJumpscare12_2(Player player) {
