...
/**
 * HorrorEntity07 - Real horror entity 7
...
public class HorrorEntity07 extends Monster {
    ...
    public static AttributeSupplier.Builder createAttributes() { return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 55.0).add(Attributes.MOVEMENT_SPEED, 0.32).add(Attributes.ATTACK_DAMAGE, 11.0).add(Attributes.FOLLOW_RANGE, 32.0); }
    ...
    public void chasePlayer2_7(Player player) {
        if (player == null || level.isClientSide) return;
    ...
    public void ambushInDark4_7(Player player) {
    ...
    public void chasePlayer5_7(Player player) {
        if (player == null || level.isClientSide) return;
        double predX = player.getX() + player.getDeltaMovement().x * 15;
        double predZ = player.getZ() + player.getDeltaMovement().z * 15;
        Vec3 targetPos = new Vec3(predX, player.getY(), predZ);
    ...
    public void dropJumpscare8_7(Player player) {
    ...
    public void chasePlayer11_7(Player player) {
        if (player == null || level.isClientSide) return;
        double predX = player.getX() + player.getDeltaMovement().x * 15;
        double predZ = player.getZ() + player.getDeltaMovement().z * 15;
        Vec3 targetPos = new Vec3(predX, player.getY(), predZ);
    ...
    public void dropJumpscare12_7(Player player) {
        if (player == null || !isOnCeiling) return;
        if (distanceTo(player) < 5 && random.nextInt(30)==0) {
            isOnCeiling = false;
            setNoGravity(false);
    ...
    public void teleportBehind14_7(Player player) {
